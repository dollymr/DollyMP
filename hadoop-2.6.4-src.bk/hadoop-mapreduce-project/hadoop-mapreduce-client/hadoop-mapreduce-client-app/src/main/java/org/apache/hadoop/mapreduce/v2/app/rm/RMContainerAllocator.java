  ...
  */
  ...
  protected void serviceInit(Configuration conf) throws Exception {
    ...
    allocationDelayThresholdMs = conf.getInt(
        ...
        MRJobConfig.DEFAULT_MR_JOB_REDUCER_PREEMPT_DELAY_SEC) * 1000;//sec -> Mrs
  ...
  public long computeWorkload(TaskId id){
      	...
      	time = mapNum * (a + b) + reduceNum * (c + d);
      	time = time == 0 ? mapNum + reduceNum : time;
...
private static String getHost(String contMgrAddress) {
  ...
  private List<Container> getResources() throws Exception {
     ...
     */
    try {
      LOG.info("update workload and send requests!");
      updateTaskList();
      long wl = 0;
      if (taskList.size() != 0) {
  ...
  class ScheduledRequests {
    ...
    //Map<TaskAttemptId, ContainerRequest> maps1 =
    	      //new LinkedHashMap<TaskAttemptId, ContainerRequest>();
    Map<TaskAttemptId, ContainerRequest>[] mapsk =
  	      new Map[10];
    private final LinkedHashMap<TaskAttemptId, ContainerRequest> reduces = 
      new LinkedHashMap<TaskAttemptId, ContainerRequest>();

    //private LinkedHashMap<TaskAttemptId, ContainerRequest> reduces1 = 
    	      //new LinkedHashMap<TaskAttemptId, ContainerRequest>();
    ...
    private void assignMapsWithLocality(List<Container> allocatedContainers) {
      ...
      Iterator<Container> it = allocatedContainers.iterator();
      int num;
      int a = allocatedContainers.size();
      int b = maps.size();
      if(b > 0){
      int t = a/b;
      for(num = 0; num < t + 1; num++){
    	  mapsk[num] = new LinkedHashMap<TaskAttemptId, ContainerRequest>();
    	  mapsk[num] = maps;
      }
      }      
      //maps1 = maps;
      ...
      int s = 0;
      while(it.hasNext()){
            while(mapsk[s].size() > 0){
      		Container allocated = it.next();
      		Priority priority = allocated.getPriority();
      		assert PRIORITY_MAP.equals(priority);
      		TaskAttemptId tId = mapsk[s].keySet().iterator().next();
      		ContainerRequest assigned = mapsk[s].remove(tId);
            containerAssigned(allocated, assigned);
            ...
            eventHandler.handle(jce);
            if (LOG.isDebugEnabled()) {
              LOG.debug("Assigned based on optimization schedular");
      		}
      }
            s++;
      }
