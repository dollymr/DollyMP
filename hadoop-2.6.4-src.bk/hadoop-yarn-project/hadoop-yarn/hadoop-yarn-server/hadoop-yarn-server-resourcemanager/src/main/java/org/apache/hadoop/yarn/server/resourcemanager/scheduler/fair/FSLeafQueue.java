...
public class FSLeafQueue extends FSQueue {
	...
	private long effectiveWorkload = 0;
	...
	public Resource assignContainer(FSSchedulerNode node) {
		...
		// if (LOG.isDebugEnabled()) {
		// String debugInfo = "BEFORE sorting, runnable applications of size = " +
		// runnableApps.size() + ":";
		// for (FSAppAttempt sched : runnableApps)
		// debugInfo += " " + sched.getApplicationAttemptId();
		// LOG.debug(debugInfo + " in queue: " + getName());
		// }
		writeLock.lock();
		try {
		...
		if (LOG.isDebugEnabled()) {
			String debugInfo = "CLONE DEBUG: AFTER sorting, runnable and nonrunnable applications of size = " + runnableApps.size() + ":";
			for (FSAppAttempt sched : runnableApps)
				debugInfo += " " + sched.getApplicationAttemptId();
			LOG.debug(debugInfo + " in queue: " + getName());
		}
		...
		try {
			for (FSAppAttempt sched : runnableApps) {
				...
				Resource share = sched.getEffectiveShare();
				Resource consumed = sched.getCurrentConsumption();// sched.getResourceUnderUse();
				Resource remaining = Resources.subtract(share, consumed);
				if (LOG.isDebugEnabled()) {
					LOG.debug("CLONE DEBUG: workload = " + sched.getEffectiveWorkload());
					LOG.debug("CLONE DEBUG: intended and currently using resources of " + sched.getApplicationAttemptId() + " are: " + share
							+ " and " + consumed + ", the remaining is " + remaining);
				}
				if (remaining.getMemory() <= 0 || remaining.getVirtualCores() <= 0) {
					LOG.info("attempt " + sched.getApplicationAttemptId() + "does not have enough share");
					continue;
				}
	...
	/**
	 * compute how many copies for each unscheduled/running tasks can launch
	 * 
	 * @param available
	 * @param unscheduled
	 * @param running
	 * @return 0 for do nothing, negative for preemption, positive for launch new
	 *         copies
	 */
	// private int computeCopyNum (Resource remaining,
	// List<Resource> unscheduled,
	// List<Resource> running) {
	// if (remaining.getMemory() == 0 || remaining.getVirtualCores() == 0) {
	// return 0;
	// }
	//
	...
	//
	// } else {
	// if (unscheduled.size() > 0) { // launch unscheduled copies
	// Resource total = Resources.createResource(0, 0);
	// for (Resource res: unscheduled)
	// Resources.addTo(total, res);
	// return Math.min(remaining.getMemory()/total.getMemory(),
	// remaining.getVirtualCores()/total.getVirtualCores());
	//
	// } else if(running.size() > 0) { // clone existing copies
	// Resource total = Resources.createResource(0, 0);
	// for (Resource res: running)
	// Resources.addTo(total, res);
	// return Math.min(remaining.getMemory()/total.getMemory(),
	// remaining.getVirtualCores()/total.getVirtualCores());
	// }
	...
	private boolean isStarved(Resource share) {
		Resource desiredShare = Resources.min(FairScheduler.getResourceCalculator(), scheduler.getClusterResource(), share,
				getDemand());
		return Resources.lessThan(FairScheduler.getResourceCalculator(), scheduler.getClusterResource(), getResourceUsage(),
				desiredShare);
	}
	...
	public void updateEffectiveWorkload() {
		...
		try {
			for (FSAppAttempt app : runnableApps) {
				app.updateEffectiveWorkload();
				effectiveWorkload += app.getEffectiveWorkload();
			}
			for (FSAppAttempt app : nonRunnableApps) {
				app.updateEffectiveWorkload();
				effectiveWorkload += app.getEffectiveWorkload();
		...
		if (LOG.isInfoEnabled()) {
			LOG.info("The updated effectiveWorkload for " + getName() + " is " + effectiveWorkload);
		}
	}
}
