...
public abstract class TaskAttemptImpl implements
  ...
  private static final StateMachineFactory
    ...
    = new StateMachineFactory
     ...
     .addTransition(TaskAttemptStateInternal.NEW, TaskAttemptStateInternal.ASSIGNED,
             TaskAttemptEventType.TA_CLONE, new CloneContainerAssignedTransition())
  ...
  static class CloneContainerAssignedTransition implements
  SingleArcTransition<TaskAttemptImpl, TaskAttemptEvent> {
	  @SuppressWarnings("unchecked")
	    @Override
	    public void transition(TaskAttemptImpl taskAttempt, 
	        TaskAttemptEvent event) {
	      final CloneTaskAttemptContainerAssignedEvent cEvent = 
	    	(CloneTaskAttemptContainerAssignedEvent) event;
	      Container container = cEvent.getContainer();
	      taskAttempt.container = container;
	      // this is a _real_ Task (classic Hadoop mapred flavor):
	      taskAttempt.remoteTask = taskAttempt.createRemoteTask();
	      ...
	      // TODO: 'taskAttempt' and the new cloned task need to register that
	      // they are the relevant task attempts
	      
	      // This was done when the clone attempt created before call
	      // CloneTaskAttemptContainerAssignedEvent
	  }
  }
  ...
  private static class DeallocateContainerTransition implements
    ...
    public void transition(TaskAttemptImpl taskAttempt, 
      ...
      if (event instanceof TaskAttemptKillEvent) {
    	if(event instanceof KillTaskAttemptOnUnusableEvent){
    		ContainerId unusableNodeContainer = ((KillTaskAttemptOnUnusableEvent) event).getContainerId();
    		if(!taskAttempt.getAssignedContainerID().equals(unusableNodeContainer)){
    			return;
    		}
    	}
