...
public abstract class RMContainerRequestor extends RMCommunicator {
  ...
  private long cloneCopiesLimitNumber = 3;
  private int workloadRatio = 1;
  ...
  static class ContainerRequest {
    ...
    public String toString() {
      ...
      sb.append("CopyNumber[").append(copiesNumber).append("]");
      sb.append("Weight[").append(weight).append("]");
      return sb.toString();
    }
  }
  
  protected void updateWorkloadForAsks(long workload) {
	  for (ResourceRequest req : ask){
		  req.setWorkload(workload);
	  }
  }
  
  protected void updateCopiesNumberForAsks() {
	  for (ResourceRequest req : ask){
		  req.setCopiesNumber(cloneCopiesLimitNumber);
	  }
  }
  
  protected long getCloneCopiesLimit(){
	  return this.cloneCopiesLimitNumber;
  }
  protected int getWeight(){
	  return this.weight;
  }
  protected int getWorkloadRatio(){
	  return workloadRatio;
  }
  
  protected void updateWeightForAsks() {
	  for (ResourceRequest req : ask){
		  req.setWeight(weight);
	  }
  }
  
  protected void updateMapTaskListForAsks(Map<TaskAttemptId, ContainerRequest> maps) {
	  String tmp ="";
	  for (ResourceRequest req : ask){
		  if(req.getPriority().getPriority()==20){
			  for(TaskAttemptId TAId: maps.keySet()){
				  tmp+=(" "+TAId.getTaskId().toString());
			  }
			  req.setTaskList(tmp);
		  }
	  }
  }
  protected void updateFailedMapTaskListForAsks(LinkedList<TaskAttemptId> earlierFailedMaps) {
	  String tmp ="";
	  for (ResourceRequest req : ask){
		  if(req.getPriority().getPriority()==5){
			  for(TaskAttemptId TAId: earlierFailedMaps){
				  tmp+=(" "+TAId.getTaskId().toString());
			  }
			  req.setTaskList(tmp);
		  }
  ...
  protected void updateReduceTaskListForAsks(LinkedHashMap<TaskAttemptId, ContainerRequest> reduces) {
	  String tmp ="";
	  for (ResourceRequest req : ask){
		  if(req.getPriority().getPriority()==10){
			  for(TaskAttemptId TAId: reduces.keySet()){
				  tmp+=(" "+TAId.getTaskId().toString());
			  }
			  req.setTaskList(tmp);
		  }
  ...
  protected void serviceInit(Configuration conf) throws Exception {
    ...
    cloneCopiesLimitNumber = 
      conf.getInt(MRJobConfig.MR_CLONE_ATTEMPTS_COPYIES_LIMIT_KEY, 
    		      MRJobConfig.DEFAULT_MR_CLONE_ATTEMPTS_COPYIES_LIMIT_KEY);
    weight = 
      conf.getInt(MRJobConfig.MR_CLONE_ATTEMPTS_WEIGHT_KEY, 
    		  MRJobConfig.DEFAULT_MR_CLONE_ATTEMPTS_WEIGHT_KEY);
    workloadRatio = 
      conf.getInt(MRJobConfig.MR_CLONE_ATTEMPTS_WORKLOAD_RATIO_KEY, 
    		  MRJobConfig.DEFAULT_MR_CLONE_ATTEMPTS_WORKLOAD_RATIO_KEY);
    ...
    LOG.info("cloneCopiesLimitNumber is " + cloneCopiesLimitNumber);
    LOG.info("weight is " + weight);
    LOG.info("workloadRatio is " + workloadRatio);
  }
  ...
  protected AllocateResponse makeRemoteRequest() throws YarnException, IOException {
		ResourceBlacklistRequest blacklistRequest = ResourceBlacklistRequest
				.newInstance(new ArrayList<String>(blacklistAdditions), new ArrayList<String>(blacklistRemovals));
		AllocateRequest allocateRequest = AllocateRequest.newInstance(lastResponseID, super.getApplicationProgress(),
				new ArrayList<ResourceRequest>(ask), new ArrayList<ContainerId>(release), blacklistRequest);
		...
		if (ask.size() > 0 || release.size() > 0) {
			LOG.info("getResources() for " + applicationId + ":" + " ask=" + ask.size() + " release= " + release.size()
					+ " newContainers=" + allocateResponse.getAllocatedContainers().size() + " finishedContainers="
					+ allocateResponse.getCompletedContainersStatuses().size() + " resourcelimit=" + availableResources
		...
		if (blacklistAdditions.size() > 0 || blacklistRemovals.size() > 0) {
			LOG.info("Update the blacklist for " + applicationId + ": blacklistAdditions=" + blacklistAdditions.size()
	...
	protected AllocateResponse makeRemoteRequest(long workload) throws YarnException, IOException {
		ResourceBlacklistRequest blacklistRequest = ResourceBlacklistRequest
				.newInstance(new ArrayList<String>(blacklistAdditions), new ArrayList<String>(blacklistRemovals));
		AllocateRequest allocateRequest = AllocateRequest.newInstance(lastResponseID, super.getApplicationProgress(),
				new ArrayList<ResourceRequest>(ask), new ArrayList<ContainerId>(release), blacklistRequest);
		if(ask.size() != 0){ 
			for (ResourceRequest r : ask) {
				LOG.info("make remote request: " + r + " the workload is " + workload + " the task list is " + r.getTaskList());
			}
		}else{
			LOG.info("ask size is 0");
		}
		AllocateResponse allocateResponse = scheduler.allocate(allocateRequest);
		lastResponseID = allocateResponse.getResponseId();
		availableResources = allocateResponse.getAvailableResources();
            ...
            if (!ask.remove(req)) {
              ...
              ResourceRequest zeroedRequest =
                  ResourceRequest.newInstance(req.getPriority(),
                    ...
                    req.getNumContainers(), req.getRelaxLocality(), req.getWorkload());
  ...
  protected void addContainerReq(ContainerRequest req) {
    ...
    for (String host : req.hosts) {
      ...
      if (!isNodeBlacklisted(host)) {
        addResourceRequest(req.priority, host, req.capability, req.getWorkload(), req.attemptID.getTaskId());
      }      
    }

    // Nothing Rack-local for now
    for (String rack : req.racks) {
      addResourceRequest(req.priority, rack, req.capability, req.getWorkload(), req.attemptID.getTaskId());
    }

    // Off-switch
    addResourceRequest(req.priority, ResourceRequest.ANY, req.capability, req.getWorkload(), req.attemptID.getTaskId());
  }
  ...
  private void addResourceRequest(Priority priority, String resourceName,
      Resource capability, long workload, TaskId TaskId) {
    ...
    remoteRequest.setWorkload(workload);
    remoteRequest.setNumContainers(remoteRequest.getNumContainers() + 1);
    remoteRequest.setTaskList(remoteRequest.getTaskList()+" "+TaskId.toString());
    if (isNew){
    	LOG.info("create new remote request: "+ remoteRequest);
    }else{
    	LOG.info("update remote request: "+ remoteRequest);
    }
  ...
  private void addResourceRequestToAsk(ResourceRequest remoteRequest) {
    ...
    if(LOG.isInfoEnabled()){
    	LOG.info("Add Resource Request whose host is : "+remoteRequest.getNodeLabelExpression()
    	+"continer number is: "+ remoteRequest.getNumContainers()+" to the 'ask' resource request list");
    }
  }
