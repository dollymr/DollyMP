...
import org.apache.hadoop.yarn.server.resourcemanager.scheduler.tetris.CapacitySchedulerConfiguration.QueueMapping;
import org.apache.hadoop.yarn.server.resourcemanager.scheduler.tetris.CapacitySchedulerConfiguration.QueueMapping.MappingType;
...
public class CapacityScheduler extends
  ...
  static final Comparator<FiCaSchedulerApp> applicationComparatorWithTetrisScore = 
    new Comparator<FiCaSchedulerApp>() {
	@Override
	public int compare(FiCaSchedulerApp a1, FiCaSchedulerApp a2) {
	  double a1priority = ((double)a1.getAppTetrisScore())/((double)a1.getAppWeight());
	  double a2priority = ((double)a2.getAppTetrisScore())/((double)a2.getAppWeight());
	  if (a1priority == a2priority) {
	    return a1.getApplicationId().compareTo(a2.getApplicationId());
	  } else {
	    return a1priority > a2priority ? 1 : 
  ...
  @Override
  public Comparator<FiCaSchedulerApp> getApplicationComparator(int number) {
	if(number == 0){
		return applicationComparatorWithPriority;
	}else if(number == 1){
		return applicationComparatorWithoutPriority;
	}else{
		return applicationComparatorWithTetrisScore;
	}
  }
