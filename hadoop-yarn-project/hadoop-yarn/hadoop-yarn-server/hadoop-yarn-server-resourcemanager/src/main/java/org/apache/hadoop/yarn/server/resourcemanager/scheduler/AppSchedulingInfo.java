...
public class AppSchedulingInfo {
  ...
  private long appEffectiveWorkload = 0;
  private long appCopiesNumber = 0;
  private long appWeight = 1;
  
  //this is the cash for tasks' alignment
  private HashMap<String, Double> alignments =
  ...
  //private Map<String, Integer> taskList = new HashMap<String, Integer>();
  ...
  public void setAppEffevtiveWorkload(long workload){
  ...
  public void setAppCopiesNumber(long copiesNumber){
	  this.appCopiesNumber = copiesNumber;
  }
  
  public synchronized String getAppTaskList(){
	  return appTaskList;
  }
  
  public void setAppTaskList(String taskList){
	  this.appTaskList = taskList;
  }
  ...
  public void setAppTaskAlignments(HashMap<String, Double> alignments){
  ...
  public void setAppWeight(long weight){
  ...
  public synchronized boolean isOnlyAny(){
	return onlyAny;
  }
  ...
  synchronized public void updateResourceRequests(
    ...
    for (ResourceRequest request : requests) {
      ...
      if (resourceName.equals(ResourceRequest.ANY)) {
        ...
        appEffectiveWorkload = appEffectiveWorkload == request.getWorkload() 
    			? appEffectiveWorkload : request.getWorkload();
    	if(request.getCopiesNumber()<0){
    		//appCopiesNumber = appCopiesNumber;
    	}else if(request.getCopiesNumber()>3){
    		appCopiesNumber = 3;
    	}else{
    		appCopiesNumber = appCopiesNumber == request.getCopiesNumber()
        			? appCopiesNumber : request.getCopiesNumber();
    	}
    	if(request.getPriority().getPriority()!=0&&request.getPriority().getPriority()!=5&&request.getPriority().getPriority()!=10){
    		appTaskList = appTaskList.equals(request.getTaskList())
    			? appTaskList : request.getTaskList();
    		alignments.clear();
    		StringTokenizer st = new StringTokenizer(appTaskList);
    		while (st.hasMoreTokens()) {
    	      String taskId = st.nextToken();
    	      if(taskId != null && !taskId.equals("null")){
    	    	  alignments.put(taskId, calculateAlignmentScore(request.getCapability()));
    	      }
    	    }
    	}
        if(LOG.isDebugEnabled()){
        	LOG.debug("Update:" + " application=" + applicationId + " workload=" + appEffectiveWorkload);
            LOG.debug("Update:" + " application=" + applicationId + " copiesNumber=" + appCopiesNumber);
            LOG.debug("Update:" + " application=" + applicationId + " TaskList=" + appTaskList);
        }
      }else{
    	  this.onlyAny = false;
      }
  ...
  synchronized public List<ResourceRequest> allocate(NodeType type, SchedulerNode node, Priority priority,
			ResourceRequest request, Container container, boolean isForClone) {
		List<ResourceRequest> resourceRequests = new ArrayList<ResourceRequest>();
		if (type == NodeType.NODE_LOCAL) {
			allocateNodeLocal(node, priority, request, container, resourceRequests, isForClone);
		} else if (type == NodeType.RACK_LOCAL) {
			allocateRackLocal(node, priority, request, container, resourceRequests, isForClone);
		} else {
			allocateOffSwitch(node, priority, request, container, resourceRequests, isForClone);
		}
		QueueMetrics metrics = queue.getMetrics();
		if (pending) {
			// once an allocation is done we assume the application is
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("allocate: applicationId=" + applicationId + " container=" + container.getId() + " host="
					+ container.getNodeId().toString() + " user=" + user + " resource=" + request.getCapability());
		}
		if (LOG.isInfoEnabled() && isForClone ){
			LOG.info("This is one duplicate container");
		}
		metrics.allocateResources(user, 1, request.getCapability(), true);
		return resourceRequests;
	}
  ...
  /**
	 ...
	 *          resources allocated to the application
	 */
	synchronized private void allocateNodeLocal(SchedulerNode node, Priority priority, ResourceRequest nodeLocalRequest,
			Container container, List<ResourceRequest> resourceRequests, boolean isForClone) {
		// Update future requirements
		if (!isForClone) {
			nodeLocalRequest.setNumContainers(nodeLocalRequest.getNumContainers() - 1);
			if (nodeLocalRequest.getNumContainers() <= 0) {
				if (LOG.isDebugEnabled())
					LOG.debug("Before removednode local request is removed: " + nodeLocalRequest + " and request is "
							+ getAllResourceRequests());
				this.requests.get(priority).remove(node.getNodeName());
				if (LOG.isDebugEnabled())
					LOG.debug("After removednode the request is " + getAllResourceRequests());
			}
		}
		// Update cloned NodeLocal, RackLocal and OffRack requests for recovery
		resourceRequests.add(cloneResourceRequest(nodeLocalRequest));
	 ...
	 *          resources allocated to the application
	 */
	synchronized private void allocateRackLocal(SchedulerNode node, Priority priority, ResourceRequest rackLocalRequest,
			Container container, List<ResourceRequest> resourceRequests, boolean isForClone) {
		// Update future requirements
		if (!isForClone) {
			if (rackLocalRequest.getNumContainers() > 0)
	 ...
	 *          resources allocated to the application
	 */
	synchronized private void allocateOffSwitch(SchedulerNode node, Priority priority, ResourceRequest offSwitchRequest,
			Container container, List<ResourceRequest> resourceRequests, boolean isForClone) {
		// Update future requirements
		if (!isForClone) {
			decrementOutstanding(offSwitchRequest);
  ...
  private double calculateAlignmentScore(Resource capacity){
	  int mem = capacity.getMemory();
	  int cpu = capacity.getVirtualCores();
	  return ((double)mem/1024) + cpu;  // how to normalize the 
  }
}
