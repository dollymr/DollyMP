...
package org.apache.hadoop.yarn.server.resourcemanager.scheduler.srptmsc;
...
import org.apache.hadoop.yarn.server.resourcemanager.scheduler.srptmsc.CapacitySchedulerConfiguration.QueueMapping;
import org.apache.hadoop.yarn.server.resourcemanager.scheduler.srptmsc.CapacitySchedulerConfiguration.QueueMapping.MappingType;
import org.apache.hadoop.yarn.server.resourcemanager.scheduler.common.QueueEntitlement;
import org.apache.hadoop.yarn.server.resourcemanager.scheduler.common.fisrpt.FiCaSchedulerApp;
import org.apache.hadoop.yarn.server.resourcemanager.scheduler.common.fisrpt.FiCaSchedulerNode;
...
public class CapacityScheduler extends
  ...
  static final Comparator<CSQueue> queueComparator = new Comparator<CSQueue>() {
    ...
    public int compare(CSQueue q1, CSQueue q2) {
      ...
      double q1priority = ((double)q1.getWeight())/((double)q1.getFinishTime());
      double q2priority = ((double)q2.getWeight())/((double)q2.getFinishTime());
      if (q1priority > q2priority) {
        return -1;
      } else if (q1priority > q2priority) {
        return 1;
      }
      // if the 'priority's are same, then use the original method to compare
  ...
  static final Comparator<FiCaSchedulerApp> applicationComparatorWithPriority = 
    new Comparator<FiCaSchedulerApp>() {
    @Override
    public int compare(FiCaSchedulerApp a1, FiCaSchedulerApp a2) {
      double a1priority = ((double)a1.getAppEffectiveWorkload())/((double)a1.getAppWeight());
      double a2priority = ((double)a2.getAppEffectiveWorkload())/((double)a2.getAppWeight());
      if (a1priority == a2priority) {
        return a1.getApplicationId().compareTo(a2.getApplicationId());
      } else {
    	  return a1priority < a2priority ? -1 : 
    		  a1priority > a2priority ? 1 : 0;
      }
    }
  };
  static final Comparator<FiCaSchedulerApp> applicationComparatorWithoutPriority = 
    new Comparator<FiCaSchedulerApp>() {
	@Override
	public int compare(FiCaSchedulerApp a1, FiCaSchedulerApp a2) {
	  return a1.getApplicationId().compareTo(a2.getApplicationId());
  ...
  @Override
  public Comparator<FiCaSchedulerApp> getApplicationComparator(boolean needPriority) {
	if(needPriority){
		return applicationComparatorWithPriority;
	}else{
		return applicationComparatorWithoutPriority;
	}
  }
  ...
  static void schedule(CapacityScheduler cs) {
    ...
    Map<ApplicationId, SchedulerApplication<FiCaSchedulerApp>> apps = cs.getSchedulerApplications();
    int count = 0;
    while(count<10){
      int current = 0;
      int start = random.nextInt(nodes.size());
      for(SchedulerApplication app: apps.values()){
        app.getCurrentAppAttempt().setIfHOSTCloneRound(true);;
      }
      ...
      for(SchedulerApplication app: apps.values()){
    	  app.getCurrentAppAttempt().setIfHOSTCloneRound(false);;
      }
      //Assign Rack or Any clone
      current=0;
      for (FiCaSchedulerNode node : nodes) {
          if (current++ >= start) {
            cs.allocateContainersToNode(node);
  ...
  private synchronized void addApplication(ApplicationId applicationId,
    String queueName, String user, boolean isAppRecovering, long workloadRatio,
    int mapperNum, int weight) {
    ...
    application.setWorkloadRatio(workloadRatio);
    application.setMapperNumber(mapperNum);
  ...
  private synchronized void addApplicationAttempt(
    ...
    FiCaSchedulerApp attempt =
        new FiCaSchedulerApp(applicationAttemptId, application.getUser(),
          ...
          application.getWorkloadRatio()*application.getMapperNumber(), application.getWeight());
  ...
  private synchronized void allocateContainersToNode(FiCaSchedulerNode node) {
    ...
    if (reservedContainer != null) {
      ...
      List<CSAssignment> assignments = queue.assignContainers(clusterResource, node,
          false, CSQueueUtils.getAbsoluteMaxAvailCapacity(
        		  queue.resourceCalculator, clusterResource, queue));
      for(CSAssignment assignment : assignments){
    ...
    if (node.getReservedContainer() == null) {
      if (calculator.computeAvailableContainers(node.getAvailableResource(),
        ...
        root.assignContainers(clusterResource, node, false, 1);
      }
    } else {
    ...
    case APP_ADDED:
    {
      ...
      if (queueName != null) {
        addApplication(appAddedEvent.getApplicationId(),
            ...
            appAddedEvent.getWorkloadRatio(),
            appAddedEvent.getMapperNumber(),
            appAddedEvent.getWeight());
      }
    }
    break;
    case APP_REMOVED:
