...
public class FSParentQueue extends FSQueue {
  ...
  public void updateEffectiveWorkload() {
	...
	effectiveWorkload = 0;
	for (FSQueue childQueue : childQueues) {
		childQueue.updateEffectiveWorkload();;
		float toAdd = childQueue.getEffectiveWorkload();
		if (LOG.isDebugEnabled()) {
			LOG.debug("Counting workload from " + childQueue.getName() + " " + 
					toAdd + "; Total workload for " + getName() +
					" now " + effectiveWorkload);
		}
		effectiveWorkload += toAdd;
	}
	if (LOG.isDebugEnabled()) {
		LOG.debug("The updated effectiveWorkload for " + getName() +
				" is " + effectiveWorkload);
	}
  }
}
