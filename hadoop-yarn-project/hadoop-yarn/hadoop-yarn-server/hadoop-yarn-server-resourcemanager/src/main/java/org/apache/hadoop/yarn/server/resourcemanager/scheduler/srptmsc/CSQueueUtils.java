...
class CSQueueUtils {
  ...
  public static void updateQueueStatistics(
   ...
   public static float getAbsoluteMaxAvailCapacity(
      ...
      float siblingUsedCapacity = Resources.ratio(
                 ...
                 parentResource);
      //my max avail is the lesser of twice of my max capacity, 
      //what is unused from my parent and the deserved resource number
      //by my siblings (if they are beyond their base capacity)
      float maxAvail = Math.min(
        2 * queue.getMaximumCapacity(),
        1.0f - siblingUsedCapacity);
      //and, multiply by parent to get absolute (cluster relative) value
      float absoluteMaxAvail = Math.min(maxAvail * parentMaxAvail,
    		  deservedResourceCalculator(queue));
  ...
  //This is the calculator of the deserved Resource
  public static float deservedResourceCalculator(CSQueue queue){
	float result = 0.0f;
    //Get all applications weights sum
	CSQueue findRoot = queue;
	long allAppWeight = 0;
	while(findRoot.getParent() != null){
	  findRoot = findRoot.getParent();
	  allAppWeight = findRoot.getWeight();
	}
	if(allAppWeight == 0){ //no app registered.
		return result;
	}
	result = Math.min(2 * queue.getAbsoluteMaximumCapacity(), 
			(float)queue.getWeight()/(float)allAppWeight);
	return result;
  }
}
